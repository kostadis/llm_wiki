...
describe("resolveBrokenLinksByEmbedding", () => {
  function broken(page: string, brokenTarget: string): LintResult {
    return { type: "broken-link", severity: "warning", page, detail: "", brokenTarget }
  }

  it("fills suggestedTarget for an otherwise-unresolved broken link", async () => {
    mockListDirectory.mockResolvedValue([node("a.md"), node("b.md")])
    mockReadFile.mockImplementation(async (p) => `body of ${p}`)
    mockFetchEmbedding.mockImplementation(async (text) => {
      if (text.startsWith("b.md")) return [1, 0, 0]
      if (text === "foobar") return [0.9, 0, 0] // broken text near b
      return [0, 1, 0]
    })

    const out = await resolveBrokenLinksByEmbedding("/project", [broken("a.md", "foobar")], embeddingConfig())

    expect(out[0].suggestedTarget).toBe("b")
    expect(out[0].detail).toContain("semantic match")
  })

  it("does not touch a broken link that already has a lexical repoint", async () => {
    mockFetchEmbedding.mockClear()
    const already = { ...broken("a.md", "foobar"), suggestedTarget: "thing.md" }
    const out = await resolveBrokenLinksByEmbedding("/project", [already], embeddingConfig())
    expect(out[0].suggestedTarget).toBe("thing.md")
    expect(mockFetchEmbedding).not.toHaveBeenCalled()
  })
})

describe("resolveNoOutlinksByEmbedding", () => {
  it("attaches the closest related page as the forward-link target", async () => {
    mockListDirectory.mockResolvedValue([node("a.md"), node("b.md")])
    mockReadFile.mockImplementation(async (p) => `body of ${p}`)
    mockFetchEmbedding.mockImplementation(async (text) =>
      text.startsWith("a.md") ? [1, 0, 0] : [0.95, 0.05, 0],
    )

    const noOutlinks: LintResult[] = [
      { type: "no-outlinks", severity: "info", page: "a.md", detail: "No outbound links." },
    ]
    const out = await resolveNoOutlinksByEmbedding("/project", noOutlinks, embeddingConfig())

    expect(out[0].affectedPages).toEqual(["b.md"])
    expect(out[0].detail).toContain("[[b]]")
  })
})
